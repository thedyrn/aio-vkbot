...
@pytest.fixture
def msg_obj(new_message_update):
    message = new_message_update['object']['message']
...
def test_message_from_dict(new_message_update, msg_obj):
    msg_from_dict = Message.from_dict(new_message_update['object']['message'])
    assert msg_from_dict == msg_obj


def test_update_from_dict(new_message_update, msg_obj):
    upd = Update.from_dict(new_message_update)
